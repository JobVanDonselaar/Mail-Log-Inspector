...
- De extra `Excel export`-knop exporteert de top 25/50/100/1000/5000 of `Alles` van langste aflevertijden (eerste event tot afgeleverd) voor de gekozen periode, met samenvatting, volledige archiefhistorie per mail en filtercontext. In het overzicht staat nu een kolom `Waarom traag?` met automatische duiding (zoals retries, tijdelijke uitstelcodes, bouncepatroon en dominante SMTP-code).
- Voor archiefhistorie gebruikt de app een import-index (`import_mail_lookup`) op tracking-key + ontvanger. Daardoor kan historie meestal gericht uit een kleine set relevante archieven komen in plaats van brede scans; bij oudere imports zonder index valt de app veilig terug op de bestaande periodeselectie.
...
- Dubbelklikken op een afzenderadres opent `Zoeken` met dat adres als afzenderfilter en de periode van de melding, en start de zoekactie meteen. Zo zijn de bounces achter het getal direct te bekijken.
...
- Bij de verzendinstellingen kan optioneel één BCC-adres worden ingesteld. Elke bouncemelding gaat dan onzichtbaar ook naar dat adres.
